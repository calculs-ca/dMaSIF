...
def get_single(pdb_id: str,chains: list):
    ...
    pdb = PDBParser().get_structure(pdb_id, pdb_filename)
    for chain in pdb.get_chains():
        chain_id=chain.get_id()
        out_filename = pdb_dir/f"{pdb_id}_{chain_id}.pdb"
        extractPDB(pdb_filename, str(out_filename), chain_id)
        protein = load_structure_np(out_filename,center=False)
        np.save(npy_dir / f"{pdb_id}_{chain_id}_atomxyz", protein["xyz"])
        np.save(npy_dir / f"{pdb_id}_{chain_id}_atomtypes", protein["types"])
